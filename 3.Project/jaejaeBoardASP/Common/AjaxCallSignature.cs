...
namespace jaejaeBoardASP.Common
{
    public class AjaxCallSignature
    {
		public AjaxCallSignature(HttpContext context)
		{
			...
			if (context.Request.RequestType.ToUpper() == "POST")
			{
				...
				foreach (var item in requestParams)
				{
					...
					else if (item.ToLower().Equals("returntype"))
		...
		private object ProcessProperty(string propertyName, Type propertyType, int depth)
		{
			...
			else if (propertyType.IsGenericType && propertyType.Namespace.Equals("System.Collections.Generic"))
			{
				depth++;
				return HydrateGeneric(propertyName, propertyType, depth);
			}
		...
		private object HydrateGeneric(string propertyName, Type propertyType, int depth)
		{
			var arguments = (IList)Activator.CreateInstance(propertyType);
			var argumentObject = Activator.CreateInstance(propertyType.GetGenericArguments()[0]);
			...
			foreach (var p in objectProperties)
			{
				...
				if (!objectIndex.Equals(nestedProperties[1]))
				{
					arguments.Add(argumentObject);
					argumentObject = Activator.CreateInstance(propertyType.GetGenericArguments()[0]);
					objectIndex = nestedProperties[1];
				}
				argumentObject.GetType().GetProperty(nestedProperties[depth + 1]).SetValue(argumentObject, ProcessProperty(propertyName + "+" + nestedProperties[depth] + "+" + nestedProperties[depth + 1], argumentObject.GetType().GetProperty(nestedProperties[depth + 1]).PropertyType, depth + 1), null);
			}
		...
		public object HydrateValue(string propertyName, Type propertyType)
		{
			if (args.Keys.Contains(propertyName))
			{
				...
				{
					if (args[propertyName] != null && !args[propertyName].Equals("undefined"))
